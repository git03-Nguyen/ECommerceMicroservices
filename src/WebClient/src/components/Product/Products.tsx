...
const Products = ({ products, loading, error }: ProductState) => {
  ...
  const handleDelete = (producdId: number) => {
    ...
    <>
          ...
          <>
            ...
            >
              {loadedProducts?.length > 0 && loadedProducts.map((product) => (
                <Grid item key={product.productId} xs={8} sm={4} md={4} lg={2}>
                  <Card
      ...
      {isEditModalOpen && (
        <EditProductModal
          ...
          product={selectedProduct || { productId: 0, name: "", price: 0, description: "", categoryId: 0, stock: 0, imageUrl: "" }}
