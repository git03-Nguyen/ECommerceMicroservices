  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const action = await dispatch(
        createProduct({ name, description, price, categoryId, stock, imageUrl })
      );
          ...
          <Grid container spacing={2} marginTop={1}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                id="categoryId"
                value={categoryId}
                onChange={(cate) => setCategoryId(parseInt(cate.target.value))}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                id="stock"
                value={stock}
                onChange={(s) => setStock(parseInt(s.target.value))}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
