...
public class GetProductsHandler : IRequestHandler<GetProductsQuery, GetProductsResponse>
{
    ...
    public async Task<GetProductsResponse> Handle(GetProductsQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            nameof(Product.ProductId) => requestPayload.SortOrder == FilterConstants.Ascending ? products.OrderBy(x => x.ProductId) : products.OrderByDescending(x => x.ProductId),
            nameof(Product.Name) => requestPayload.SortOrder == FilterConstants.Ascending ? products.OrderBy(x => x.Name) : products.OrderByDescending(x => x.Name),
            nameof(Product.Price) => requestPayload.SortOrder == FilterConstants.Ascending ? products.OrderBy(x => x.Price) : products.OrderByDescending(x => x.Price),
