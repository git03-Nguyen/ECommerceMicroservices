    ...
    public async Task Handle(DeleteProductsCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (products.Count != productIds.Count())
            {
                ...
                _logger.LogWarning("Some products are missing. ProductIds: {ProductIds}",
                    string.Join(", ", missingProductIds));
            }

            _unitOfWork.ProductRepository.RemoveRange(products);
            await _unitOfWork.SaveChangesAsync(cancellationToken);
            _logger.LogInformation("Products deleted from basket items. ProductIds: {ProductIds}",
                string.Join(", ", request.Payload.ProductIds));
        }
        catch (Exception e)
        {
            _logger.LogError(e, "An error occurred while deleting products. ProductIds: {ProductIds}",
                string.Join(", ", request.Payload.ProductIds));
            throw;
        }
    }
