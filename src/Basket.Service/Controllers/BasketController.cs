...
public class BasketController : ControllerBase
{
    ...
    //     var existingItem = _unitOfWork.BasketItemRepository
    //         .GetByCondition(x => x.BasketId == id && x.ProductId == request.ProductId).FirstOrDefault();
    ...
    //     [FromServices] IBusControl busControl)
    // {
    //     // Check if basket exists
    //     var basket = await _unitOfWork.BasketRepository.GetByIdAsync(id);
    //     if (basket == null) return NotFound();
    ...
    //     if (basket.BasketItems.Count == 0) return BadRequest("Basket is empty.");
    //
    ...
    //     await busControl.Publish(checkoutBasket, cancellationToken);
    //
    //     return Ok();
    // }
}