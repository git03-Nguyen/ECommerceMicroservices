...
public class UpdateRoleHandler : IRequestHandler<UpdateRoleCommand, UpdateRoleResponse>
{
    ...
    public async Task<UpdateRoleResponse> Handle(UpdateRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        if (role == null) throw new ResourceNotFoundException("Role", request.Payload.Name);

        role.Name = request.Payload.NewName;

        // Update role
        var result = await _roleManager.UpdateAsync(role);
        if (!result.Succeeded) throw new Exception("Failed to update role: " + result.Errors);

        _logger.LogInformation("Role updated from {OldRoleName} to {NewRoleName}", request.Payload.Name,
