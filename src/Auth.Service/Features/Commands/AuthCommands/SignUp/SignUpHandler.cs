...
public class SignUpHandler : IRequestHandler<SignUpCommand, SignUpResponse>
{
    ...
    public async Task<SignUpResponse> Handle(SignUpCommand request, CancellationToken cancellationToken)
    {
        ...
        var usename = request.Payload.UserName;
        var email = request.Payload.Email;
        var password = request.Payload.Password;
        var roles = request.Payload.Roles;
        
        _logger.LogInformation("SignUpHandler.Handle: {0} - {1} - {2} - {3}", usename, email, password, roles);
        ...
        try
        {
            ...
            foreach (var role in roles)
            {
                ...
                if (existingRole == null) throw new Exception("Role does not exist");
            }
