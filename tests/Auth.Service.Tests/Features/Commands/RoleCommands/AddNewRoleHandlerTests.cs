...
public class AddNewRoleHandlerTests
{
    ...
    public void Setup()
    {
        ...
        _handler = new AddNewRoleHandler(new Mock<ILogger<AddNewRoleHandler>>().Object, _roleManagerMock.Object, _identityServiceMock.Object);
    }
    ...
    public async Task AddNewRole_WhenRoleCreated_ReturnAddNewRoleResponse()
    {
        ...
        // Act
        var result = await _handler.Handle(new AddNewRoleCommand(new AddNewRoleRequest { Name = role}), _cancellationToken);
    ...
    public async Task AddNewRole_WhenRoleNotCreated_ThrowException()
    {
        ...
        var role = _fixture.Create<string>();
        _roleManagerMock.Setup(x => x.RoleExistsAsync(role)).ReturnsAsync(false);
        _roleManagerMock.Setup(x => x.CreateAsync(It.IsAny<ApplicationRole>())).ReturnsAsync(IdentityResult.Failed(new IdentityError { Description = "Error" }));
